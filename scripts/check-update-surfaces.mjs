...
const exposedCommandNeedles = {
  components: "['components']",
  'capability-find': "['capability-find'",
  usage: "['usage']",
  conformance: "['conformance']",
  security: "['security', 'audit', '--strict']",
  agent: "['agent', 'describe']",
};
for (const command of exposed) {
  const needle = exposedCommandNeedles[command];
  add(`mcp:exposed:${command}`, Boolean(needle && coreText.includes(needle)), needle || 'missing command mapping');
}
...
for (const entry of skillEntries) {
  ...
  add(`skill:${entry.name}:reviewed-for-core`, match?.[1] === pkg.version, `${match?.[1] || 'missing'} / ${pkg.version}`);
}
...
for (const file of requiredSurfaces) {
  const ok = await readFile(path.join(root, file), 'utf8').then(() => true, () => false);
  add(`surface:${file}`, ok, file);
}
...
if (!ok) {
  console.error('TaskRail update surfaces are stale. Review MCP, skills, platform/install assets, docs, and compatibility classifications before release.');
  process.exitCode = 1;
}
