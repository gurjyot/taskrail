...
export type RuntimeKind = 'node' | 'python' | 'shell' | 'php';
...
export interface ManifestPathRule { path: string; environments?: TaskrailEnv[]; mode?: 'must-exist' | 'writable'; secret?: boolean; }
export interface DependencyManagerConfig { tool: DependencyManagerTool; lockfile?: string; manifest?: string; installCommand?: string; }
export interface RuntimeRequirement { kind: RuntimeKind; version?: string; command?: string; }
export interface ServiceUnitDefinition { name: string; kind: 'service' | 'timer'; oneshotOkay?: boolean; user?: string; staleAfterMs?: number; }
export interface ServiceManagerDefinition { type: ServiceManagerType; units: ServiceUnitDefinition[]; }
export interface MigrationHooks { checkCommand?: string; applyCommand?: string; destructive?: boolean; }
export interface DeployStrategy { type: DeployStrategyType; releaseRoot?: string; }
...
export interface BackupPolicy { retain: number; }
export interface PluginReference { name: string; module: string; }
export interface CapabilityManifest { name: string; version: string; description: string; runtime: 'node'; canonicalPath: string; requiredSharedFiles?: string[]; healthCheck?: HealthCheckDefinition; input?: string; output?: string; }
export interface CapabilityContract extends CapabilityManifest { root: string; path: string; consumers?: string[]; }
export interface FrameworkConfig { projectName: string; environment: Record<string, string | undefined>; manifest: FrameworkManifest; }
export interface EnvironmentInfo { name: TaskrailEnv; overridden: boolean; reason: string; }
...
export interface PluginContext { config: FrameworkConfig; log(event: LogEvent): void; }
export interface AutomationPlugin { name: string; setup?(context: PluginContext): void; validate?(config: FrameworkConfig): string[]; healthCheck?(): Promise<HealthResult> | HealthResult; backup?(): Promise<BackupResult> | BackupResult; rollback?(): Promise<void> | void; reviewChange?(change: ChangeReviewInput): Promise<ChangeReviewResult> | ChangeReviewResult; }
export interface ChangeReviewInput { changedFiles: string[]; protectedPaths: string[]; risk: ChangeRisk; gate: GateVerdict; deployAllowed: boolean; }
export interface ChangeReviewResult { ok: boolean; summary?: string; }
export interface DeploymentContext { sourceDir: string; deployDir: string; candidateDir: string; backupDir: string; }
export interface HealthCheckOutcome { tier: HealthTier; ok: boolean; details?: string; }
export interface DriftItem { path: string; kind: DriftKind; reason: string; }
export interface DeployState { backupPath?: string; targetPath: string; releasePath?: string; previousReleasePath?: string; currentReleaseId?: string; currentSha?: string; currentFingerprint?: string; lastKnownGoodReleasePath?: string; lastKnownGoodReleaseId?: string; lastKnownGoodSha?: string; }
